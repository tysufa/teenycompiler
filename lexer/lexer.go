...
func isKeyword(s string) bool {
	return s == token.WHILE || s == token.LET || s == token.IF || s == token.THEN || s == token.PRINT || s == token.LABEL || s == token.GOTO || s == token.ENDIF || s == token.ENDWHILE || s == token.INPUT || s == token.REPEAT
}
